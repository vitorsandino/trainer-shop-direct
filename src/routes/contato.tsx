...
import { MessageCircle, MapPin, Clock, Send, Phone, Instagram } from "lucide-react";
...
export const Route = createFileRoute("/contato")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Fale com a Pandex Store pelo WhatsApp. Atendimento em horário comercial." },
      { property: "og:title", content: "Contato — Pandex Store" },
      { property: "og:description", content: "Atendimento direto pelo WhatsApp da Pandex Store." },
    ],
  }),
...
const PRETTY_PHONE = "+55 19 99610-8105";
...
function ContatoPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const text = encodeURIComponent(
      `Olá! Vim pelo site da Pandex Store.\n\nNome: ${form.name}\nTelefone: ${form.phone || "-"}\nAssunto: ${form.subject}\n\n${form.message}`
    );
    ...
    <div className="container mx-auto px-4 py-10 md:py-16">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground md:text-base">
          Dúvidas, pedidos ou parcerias — chama a gente no WhatsApp e respondemos pessoalmente.
        </p>
      </div>
      ...
      <div className="mx-auto mt-10 grid max-w-5xl gap-4 sm:grid-cols-2">
        ...
        >
          ...
          <p className="mt-3 inline-flex items-center gap-1.5 text-sm font-bold text-[var(--whatsapp)] group-hover:underline">
            <Phone className="h-3.5 w-3.5" /> {PRETTY_PHONE}
          </p>
        </a>
      ...
      <div className="mx-auto mt-12 grid max-w-5xl gap-8 lg:grid-cols-[1fr_320px]">
        <div className="rounded-2xl border border-border bg-card p-6 md:p-8">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Vamos abrir o WhatsApp com sua mensagem pronta.</p>

          {sent && (
            <div className="mt-4 rounded-lg border border-emerald-500/40 bg-emerald-500/10 p-3 text-sm text-emerald-700">
              ✅ Pronto! Se nada abriu, fale direto pelo WhatsApp: <strong>{PRETTY_PHONE}</strong>
            </div>
          )}
          ...
          <form onSubmit={submit} className="mt-5 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Telefone (opcional)" error={errors.phone}>
                <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                  className="cinput" placeholder="(19) 9 9610-8105" />
              </Field>
            </div>
            ...
            <button
              type="submit"
              className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-[var(--whatsapp)] px-6 py-3 font-bold text-[var(--whatsapp-foreground)] shadow-lg transition hover:brightness-110 sm:w-auto"
            >
